...
static unordered_map<string,int> map1(){
    unordered_map<string,int> map1;
    return map1;
}
static unordered_map<string,int> numNeighborsPerNode = map1();
...
void KWirelessInterface::handleMessage(cMessage *msg)
{
    ...
    if (msg->isSelfMessage() && msg->getKind() == KWIRELESSINTERFACE_NEIGH_EVENT_CODE) {
        ...
        if (currentNeighbourNodeInfoList.size() > 0) {
            ...
            // Since there is no mobility involved, only save once the neighbors for the current node // 
            if( numNeighborsPerNode.find("node" + to_string(getParentModule()->getIndex()) ) == numNeighborsPerNode.end() ){
                numNeighborsPerNode.insert({"node"+ to_string(getParentModule()->getIndex()), neighCount});
            
                // Neighbor Debug (TEST)
                //EV_INFO << "Node " << getParentModule()->getIndex() << " has " << neighListMsg->getNeighbourNameListArraySize() << " neighbors";
            }
    ...
    while (iteratorAtTxNeighbourNodeInfo != atTxNeighbourNodeInfoList.end()) {
        ...
        while (iteratorCurrentNeighbourNodeInfo != currentNeighbourNodeInfoList.end()) {
            ...
            if (atTxNeighbourNodeAddress == currentNeighbourNodeAddress) {
                ...
                //double distance = sqrt( abs(pow(ownCoord.x - neighCoord.x, 2)) + abs(pow(ownCoord.y - neighCoord.y, 2)) );
                /*EV_INFO << "Current sender position: " << ownCoord << "\n"; 
                EV_INFO << "Distance from neighbor is : " << distance << "\n";
                EV_INFO << "Neighbor " << currentNeighbourNodeInfo->nodeModule << "is located at " << neighCoord << "\n";*/
                
                /*double propDelay = distance / (3 * pow(10,8)); // in seconds
                simtime_t delaySimTime = simTime() + (simtime_t) propDelay;
                
                simtime_t outRemainder = delaySimTime.remainderForUnit(SIMTIME_S);*/
                
                //saveToFile(outRemainder);
                
                // NOTE: Propagation delay is disabled
                // function parameters: sendDirect(outPktCopy, [propagation_time], [tx_time] , currentNeighbourNodeInfo->nodeModule, "radioIn");
                //sendDirect(outPktCopy, propDelay, 0 , currentNeighbourNodeInfo->nodeModule, "radioIn"); // Modified for broadcast version
                
                // End of addition for delay calculation
                
                // send to node
                sendDirect(outPktCopy, currentNeighbourNodeInfo->nodeModule, "radioIn"); // original line
                break;
            }
...
/*void KWirelessInterface::saveToFile(simtime_t delay)
{
    // Save delays to a CSV file
    string resultdir = "./simulations/out/test/";
    
    ofstream myfile;
    myfile.open (resultdir + "msg_delay.txt", ofstream::out | ofstream::app);
    myfile << delay << "\n";
    myfile.close();
...
{
    string resultdir = "./simulations/out/test/";
    // Add "-" character at EOF to separate each run results from the other.
    ...
    /* if (getParentModule()->getIndex() == 0){
        ...
        for (pair<string, int> element : numNeighborsPerNode){            
            myfile << element.first + "," + to_string(element.second) + "\n";
        }
        myfile << "-,-\n";
        myfile.close();
    } */
